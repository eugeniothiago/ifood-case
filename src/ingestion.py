...
Provides multiple download strategies with automatic fallback:
  ...
  2. urllib.request.urlretrieve (standard library fallback)
  3. wget via subprocess (fallback when Python DNS resolver fails)
  4. curl via subprocess (second fallback)

Each strategy tries both primary and fallback CloudFront domains before
moving to the next strategy. This resilience is necessary because Databricks
Community Edition clusters can have intermittent DNS resolution issues.
"""
...
from .config import taxi_file_urls
...
def _download_with_urllib(url: str, destination: Path) -> None:
    """Download using Python's built-in urllib (no external dependency).

    urllib uses a different code path from requests and may succeed when
    the requests library fails due to DNS or connection pool issues.
    """
    tmp_path = str(destination) + ".tmp"
...
def _try_all_urls(
    download_fn, urls: list[str], destination: Path, strategy_name: str
) -> bool:
    """Try downloading from each URL in turn. Returns True on success."""
    for url in urls:
        try:
            ...
            print(f"    {strategy_name} failed for {url}: {exc}")
...
def download_taxi_data(year: int, months: Iterable[int], landing_path: str) -> list[str]:
    ...
    requests -> urllib -> wget -> curl. Each strategy tries both primary
    and fallback CloudFront domains before moving to the next strategy.
    ...
    for month in months:
        urls = taxi_file_urls(year, month)
        ...
        success = False
        errors: list[str] = []
        for name, download_fn in strategies:
            if _try_all_urls(download_fn, urls, destination, name):
                print(f"  Downloaded {destination.name} via {name}")
                success = True
                break
            else:
                errors.append(f"{name}: all URLs failed")

        if not success:
            raise RuntimeError(
                f"All download strategies failed for {urls[0]}:\n  "
                + "\n  ".join(errors)
            )
